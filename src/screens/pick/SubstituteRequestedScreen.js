...
const SubstituteRequestedScreen = ({ navigation }) => {
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: Colors.WHITE }}>
            <ScrollView>
                <Title text={locale?.headings.Substituted} style={{ borderBottomWidth: 0, alignSelf: 'center' }} />
                ...
                <View style={{ alignItems: 'center', marginHorizontal: 32, marginVertical: 30 }}>
                    ...
                    <Text style={[Typography.normal15, { textAlign: 'center', marginTop: 7 }]}>{locale?.SRS_statusText}</Text>
                </View>
                <View style={{ alignItems: 'center', padding: 32, backgroundColor: Colors.offWhite }}>
                    ...
                    <Text style={[Typography.normal15, { textAlign: 'center', marginTop: 7 }]}>{locale?.SRS_infoText}</Text>
                </View>
                <Button title={locale?.SRS_button} style={{ marginVertical: 30, marginHorizontal: 60 }} onPress={() => { navigation.popToTop() }} />
            </ScrollView>
        </SafeAreaView>
    );
};
